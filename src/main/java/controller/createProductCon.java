...
@WebServlet("/api/createProduct")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
		maxFileSize = 1024 * 1024 * 10, // 10MB
		maxRequestSize = 1024 * 1024 * 50)
public class createProductCon extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String productId = (String.valueOf(randomID));
			...
			if (ownerId != null) {
				CompletableFuture<Void> asyncTask = pdao
						.createProduct(productId, name, stock, price, ownerId, type, description, imagePath)
						.thenAccept(result -> {
							try {
								if (result) {
